...
| Audio 録音、take、comping | Audio/MIDI 録音、cycle take、punch、lane/take folder、comping、複数入力を扱える | **部分**: transportまたはTrack追加から単一マイクを最大60秒録音し、dry PCMを48 kHz PCM16 WAVとしてasset-first保存できる。Audio TrackのRecord Armはruntime-onlyで同時1件、待機中なら既存Track、待機なしなら新規TrackへUndo 1回で配置する。3秒countdown後、app-wide AudioContextの同一将来frameへWorklet captureと伴奏再生をarmし、absolute frame / sequenceを検証する。input / base / output / limiterの推定latencyと-500〜+500 ms手動offsetを可変tempoへ反映し、曲頭超過はsource rangeを非破壊trimする。入力列挙 / exact指定、`devicechange`、level、手動stop / cancel、共有384 MiB planner、monitor opt-in、project切替 / close / mutation fenceを持つ。物理loopback実測、再生中の任意punch、loop recording、hot switch、長時間streaming、take/lane/comp、複数I/Oはない | P1 | shared-clockと推定補正を3OS実機でgateし、次にloopback校正とstreaming、その後cycle takes→非破壊compへ進む。multi-I/O全対応を約束しない | [Cubase recording lanes and takes](https://www.steinberg.help/r/cubase-pro/15.0/en/cubase_nuendo/topics/track_handling/track_handling_lanes_working_with_c.html), [Logic Pro recording overview](https://support.apple.com/guide/logicpro/overview-lgcp7f3af10b/mac), [Logic Pro comping](https://support.apple.com/guide/logicpro/comping-overview-lgcp317d758e/10.7/mac/11.0) |
...
6. **部分完了**: 最大60秒・単一入力のAudio Track録音、dry保存、monitor opt-in、asset-first保存、単一Record Arm、入力device列挙 / exact選択、既存TrackへのClip追記、shared-clock伴奏同期、推定＋手動latency補正、close / mutation fenceを実装した。3OS実機gate、物理loopback校正、長時間streamingを閉じてからtake/compへ進む。
...
- permission / device loss / clock不連続 / context世代変更 / cancelでは既存Project不変、disk fullはasset store失敗として採用せず、録音・保存中のproject switch / close / Project変更は明示的に拒否する。自動latencyはhost申告に基づく推定で、手動offsetを併用できる。入力hot switch、物理loopback実測、再生中の任意punch、loop recording、長時間streamingは後続境界であり、現時点で対応済みと表示しない。
...
- [ ] Batch 6a release gate: 3OS実deviceでpermission、system default / 明示device、device loss、Record Arm先への追記、disk full、monitor、close、再起動再生、shared-clock同期 / 推定latencyを確認する。物理loopback校正と長時間streamingは別incrementである。
