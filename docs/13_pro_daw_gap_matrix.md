...
|---|---|---|---:|---|---|
| プロジェクト、履歴、素材管理 | 多数のトラック種別、素材参照、プロジェクト内の音声を前提に編集する | **部分**: schema v6、SQLite自動保存、クラッシュ復旧、Undo/Redo、`.ctsproj.json` exact metadata / routing / Audio take roundtripに加え、app-owned SHA-256 Audio Assetをnative content-addressed directory / Web IndexedDBへ保存する。nativeはstaging recovery、全retained generation / draftをrootにしたGC、端末全消去を持つ。portable binary bundle、素材browser、Web generation-aware GCはない | P0 | JSONとbinaryの分離を維持し、portable bundle / asset browserを独立設計する。競合の全交換形式を直ちに再現しない | [Cubase Pro Help](https://www.steinberg.help/r/cubase-pro/15.0/en), [Logic Pro project basics](https://support.apple.com/en-ie/guide/logicpro/lgcpe9cc47b2/mac) |
...
| Automation / modulation | Track/region automation、write/read mode、parameter curve を持ち、mix や plug-in parameter を時間変化させられる | **部分（parameter-lane Read / Bypass実装済み）**: 選択中のnon-Master Trackにvolume / panの独立laneを作り、beat snap、point追加・編集・削除・全消去、outbound hold / linear curve、Undo / Redo、保存をproduction Editorから扱える。各laneのRead / Bypassを切り替えられ、Bypass中もpointを保持・編集できる一方、live / offline WAVではautomation commandを発行せずTrack scalarを使う。transport loop / 可変tempoを含む共通resolverでlive / WAVが一致する。Track全体 / global Read、write / touch / latch、Master、insert / send / tempo automation、MIDI CC / LFO modulationはない | P0 | 次はTrack全体 / global Readとwrite / touch / latch記録を独立設計する。session上書きとUndoを固めてからinsert / send / tempo、modulationへtargetを広げる | [Cubase Suspend Read](https://www.steinberg.help/r/cubase-pro/15.0/en/cubase_nuendo/topics/automation/automation_suspend_read_c.html), [Logic Pro individual automation parameter mute](https://support.apple.com/en-za/guide/logicpro/lgcpb1a3f099/10.7/mac/11.0) |
...
7. **完了済みincrement**: 選択中のnon-Master Trackのvolume / pan lane editor、beat snap、point追加・編集・削除・全消去、outbound hold / linear、parameter-lane Read / Bypass、1 gesture = 1 Undo、保存、session停止、loop / tempo / live / WAV parityを接続する。
...
- Track全体 / global automation Read、write / touch / latch、Master / insert / send / tempo parameter automation、MIDI CC / LFO modulation
- 連続tempo ramp、audio follow / Smart Tempo相当の段階的な解析
...
Batch 4ではmetadata/domain foundationに加え、Automationのlive/offline schedulingとtempo / 拍子mapのlive / WAV / MIDI / timeline接続まで実装した。application-owned assets directory、binary recovery / GC、Audio Clip playback / editingはBatch 5、volume / panのproduction lane editorとparameter-lane Read / BypassはBatch 7、tempo / 拍子mapのproduction EditorはBatch 8で接続済みである。Track全体 / global Read、write / touch / latch、連続tempo rampとaudio followは後続へ残る。
...
### Batch 7: volume / pan Automation lane Editor + Read / Bypass（実装済み）
...
- Track全体 / global Read、write / touch / latch、Master、insert / send / tempo automation、MIDI CC / LFO modulationはこのBatchに含めず、それぞれ独立した後続gateとする。
...
- accepted変更はplayheadを保持してactive sessionを停止し、次のlive / WAV / MIDI / metronome / timelineがBatch 4の共通musical-time compilerから再構築される。tempo / 拍子map entityは変えず、current schema v6へ保存する。
...
- [x] Batch 7 implementation: selected non-Masterのvolume / pan lane editor、snap、point add / edit / delete / clear、outbound hold / linear、parameter-lane Read / Bypass、1 gesture = 1 Undo、schema v6保存、session停止、loop / tempo / live / WAV parityを実装する。
...
- [ ] Batch 7後続: Track全体 / global Read、write / touch / latchを実装し、Master / insert / send / tempo automationとmodulationはtargetごとの独立gateを通す。
