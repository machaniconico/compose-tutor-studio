...
| Audio 録音、take、comping | Audio/MIDI 録音、cycle take、punch、lane/take folder、comping、複数入力を扱える。LogicのQuick Swipe CompingはAudio take folder向けで、MIDI take folderでは利用できない。外部I/OではCubaseの`Measure Delay`、Logic ProのI/O Utility `Latency Detection (Ping)`が入出力間遅延を測定する | **部分（既存Audioの非破壊comp実装済み）**: transportまたはTrack追加から単一マイクを最大60秒録音し、dry PCMを48 kHz PCM16 WAVとしてasset-first保存できる。Record Arm、shared-clock、推定 / 物理loopback実測 / 手動latency補正、入力列挙、`devicechange`、共有384 MiB planner、project / close fenceを持つ。加えてschema v5で同一Audio Track・同一時間窓の既存Clipをtake folderへまとめ、6つ目のEditor tabからrange paint、境界移動、未使用take削除、Undo / Redo、保存 / 再読込を行える。選択takeだけをlive / WAV共通plannerで鳴らし、source handle内の中心crossfadeを使う。再生中の任意punch、loop passの連続captureと自動take生成、hot switch、長時間streaming、MIDI comping、複数入力、名前付き複数comp、flattenはない | P1 | 手動group / compの回帰と3OS録音gateを維持し、次は長時間streamingと明示loop範囲を先に閉じてcycle captureを追加する。punch、MIDI comp、複数I/Oは独立gateにする | [Cubase recording lanes and takes](https://www.steinberg.help/r/cubase-pro/15.0/en/cubase_nuendo/topics/track_handling/track_handling_lanes_working_with_c.html), [Cubase assembling takes](https://www.steinberg.help/r/cubase-pro/15.0/en/cubase_nuendo/topics/track_handling/track_handling_assembling_operations_c.html), [Logic Pro record multiple takes](https://support.apple.com/guide/logicpro/record-multiple-audio-takes-lgcpb19806af/mac), [Logic Pro comping overview / Quick Swipe](https://support.apple.com/guide/logicpro/comping-overview-lgcp317d758e/mac), [Logic Pro pack existing regions](https://support.apple.com/guide/logicpro/pack-regions-into-take-folders-lgcpb194093e/mac) |
...
- Audio cycle capture、自動take folder化、punch、複数入力、名前付きcomp / flatten
...
### Batch 6: 録音 / comping / bus-send（6a・6b・6c-1実装済み）
...
- 6c-2は長時間streamingと明示loop-range UIを先に整備した後のcycle capture / 自動take生成である。任意punch、MIDI comp、複数入力、named comp / flattenも6c-1へ含めない。
...
- [ ] Batch 6c-2: 長時間streamingと明示loop-range UIを整備し、cycle passを自動take化する。任意punch / MIDI comp / 複数入力 / named comp / flattenは独立gateとする。
