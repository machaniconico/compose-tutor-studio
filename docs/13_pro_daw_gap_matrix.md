...
|---|---|---|---:|---|---|
| プロジェクト、履歴、素材管理 | 多数のトラック種別、素材参照、プロジェクト内の音声を前提に編集する | **部分**: current schema v10、SQLite自動保存、クラッシュ復旧、Undo/Redo、`.ctsproj.json` exact metadata roundtrip、app-owned SHA-256 Audio Assetに加え、canonical Projectと全ready objectを持ち運ぶ`.ctsbundle` v1をWeb/nativeで書出し・読込できる。128 MiB total、512 KiB manifest、16 MiB Project、384 MiB operation予約、全payload検証後store、全receipt後fresh-ID adoptionを持つ。素材browserとWeb generation-aware GCはない | P0 | JSON metadataとbinary repositoryの分離を維持し、portable roundtripを回帰固定する。次はasset browserとWeb orphan GCを独立設計し、競合の全交換形式を直ちに再現しない | [Cubase Pro Help](https://www.steinberg.help/r/cubase-pro/15.0/en), [Logic Pro project basics](https://support.apple.com/en-ie/guide/logicpro/lgcpe9cc47b2/mac) |
...
| Audio Asset / Audio Track | 音声をプロジェクトへ読み込み、region/event として配置・分割・trim・fade・crossfade・loop・gain 調整できる | **部分**: WAV / MP3 / M4A / AACの読込または最大60秒のマイク録音をlocalで48 kHz mono/stereo PCM16 WAVへ正規化し、app-owned content-addressed objectとして保存する。production UIから新規または既存Audio TrackへClipを配置し、move、左右trim、gain、fade、loop、split、独立duplicate、deleteができ、shared plannerでlive / WAVを一致させる。`.ctsbundle`でready assetを別repositoryへ持ち運べる。Elastic Audio Editor内には解析由来のbounded waveformもある。missing / changed表示もある。通常Arranger waveform、隣接Clip crossfade、loop中left trim / splitはない | P0 | Batch 5の非破壊source range、asset integrity、portable roundtripを回帰維持し、Arranger waveform / crossfade / persisted loop phaseは初期Elastic Audioとは分離した後続incrementにする | [Cubase Pro Help](https://www.steinberg.help/r/cubase-pro/15.0/en), [Logic Pro project basics](https://support.apple.com/en-ie/guide/logicpro/lgcpe9cc47b2/mac) |
...
9. **完了済みincrement**: `.ctsbundle` v1のcanonical codec、128 MiB total / 512 KiB manifest / 16 MiB Project / 384 MiB reservation、Web/nativeの別操作、borrowed native validation + one bounded clone、全payload validation-before-store、all-receipts-before-fresh-ID adoption、cancel atomicityを実装する。Web generation-aware orphan GCと3OS signed candidateでの大容量実測は継続gateである。
...
- Project JSON単体はbinaryを同梱しない。別端末へはcanonical Projectと全ready objectを含む`.ctsbundle` v1を使う。Web generation-aware GC、waveform / crossfade、loop中left trim / splitは既知の後続範囲である。
...
- [x] Portable Project Bundle increment: `.ctsproj.json`と分離した`.ctsbundle` v1、canonical ordering、128 MiB total / 512 KiB manifest / 16 MiB Project / 384 MiB reservation、Web/native roundtrip、full validation-before-store、all-receipts-before-fresh-ID adoption、exact cancelを実装する。
- [ ] Portable Project Bundle release gate: 3OS signed candidateで大容量境界、既存file atomic overwrite、disk full、open/save cancel、別profile roundtripを検証する。Web generation-aware orphan GCは独立hardeningとして残す。
...
- [ ] Combined仕様書とPortable Bundle docs contract、TypeScript strict、unit、component、Playwright、Rust、release preflight、desktop build がすべて成功する。
