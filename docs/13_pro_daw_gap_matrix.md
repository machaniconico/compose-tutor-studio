...
|---|---|---|---:|---|---|
| プロジェクト、履歴、素材管理 | 多数のトラック種別、素材参照、プロジェクト内の音声を前提に編集する | **部分**: schema v2、SQLite 自動保存、クラッシュ復旧、Undo/Redo、`.ctsproj.json` の exact roundtrip はある。Audio Asset の永続実体と素材プールはない | P0 | 既存の安全な Project 集約を維持し、schema v3 で Audio Asset を追加する。競合の全プロジェクト交換形式を直ちに再現しない | [Cubase Pro Help](https://www.steinberg.help/r/cubase-pro/15.0/en), [Logic Pro project basics](https://support.apple.com/en-ie/guide/logicpro/lgcpe9cc47b2/mac) |
| トラック管理と音色 | Audio / MIDI / Instrument / Group / FX / Folder などを作成・削除・複製・並べ替えでき、音源やパッチを選べる。Logic は Track Stacks も持つ | **部分**: production UIからinstrument / drumを全曲長Clip付きで追加し、non-masterの複製・並べ替え、一般Trackの削除・改名、canonical synth 4音色の選択を行える。Masterは全管理操作、名前依存のChords / Bass / Melodyは改名・削除を保護する。Audio / Bus追加、全Track改名・削除、Folder/Stack、freeze、track versionはない | P0 | schema v2の安全な部分を維持し、Batch 4で学習roleとAudio Assetを永続化、Batch 5でAudio配置・再生、Batch 6でroutingを導入する | [Cubase Pro Help](https://www.steinberg.help/r/cubase-pro/15.0/en), [Logic Pro Track Stacks](https://support.apple.com/en-ca/guide/logicpro/lgcp9bc4b63d/mac) |
...
| Tempo / 拍子 / musical time | Tempo Track、拍子イベント、tempo change、音声からの tempo 解析・追従を扱える。Logic は Smart Tempo を持つ | **部分**: project 全体の固定 BPM と固定拍子、beat-domain scheduler はある。途中の tempo / 拍子変更、tempo map、audio follow はない | P0 | schema v3 に tempo/signature event と単一の beat↔seconds 変換を追加し、live / WAV / MIDI / UI で共有する。Smart Tempo 相当は P1 | [Cubase Tempo Track](https://www.steinberg.help/r/cubase-pro/15.0/en/cubase_nuendo/topics/editing_tempo_and_signature/editing_tempo_tempo_track_c.html), [Cubase Signature Track](https://www.steinberg.help/r/cubase-pro/15.0/en/cubase_nuendo/topics/tracks_about/tracks_about_signature_track_c.html), [Logic Pro tempo overview](https://support.apple.com/guide/logicpro/tempo-overview-lgcp3c2e8ef9/mac), [Logic Pro Smart Tempo](https://support.apple.com/guide/logicpro/smart-tempo-overview-lgcp9281e70c/10.7/mac/11.0) |
| Audio Asset / Audio Track | 音声をプロジェクトへ読み込み、region/event として配置・分割・trim・fade・crossfade・loop・gain 調整できる | **未実装**: `audio` Track、`audio` Clip、`audioAssetId` は将来用の型だけ。音声はボーカルカット用の一時入力で、Project に配置も再生もされない | P0 | schema v3 の Asset、app-owned assets directory、Audio Clip の source range を先に作る。最初は非破壊 trim / move / gain / fade と再生まで | [Cubase Pro Help](https://www.steinberg.help/r/cubase-pro/15.0/en), [Logic Pro project basics](https://support.apple.com/en-ie/guide/logicpro/lgcpe9cc47b2/mac) |
...
| Automation / modulation | Track/region automation、write/read mode、parameter curve を持ち、mix や plug-in parameter を時間変化させられる | **未実装**: `automation` Clip type は予約だけで、target、point、補間、lane UI、再生時 parameter scheduling がない | P0 | schema v3 に automation target と point/interpolation を追加する。volume/pan から開始し、insert/send/tempo へ広げる。UI と offline render は同じ resolver を使う | [Cubase Automation](https://www.steinberg.help/r/cubase-pro/15.0/en/cubase_nuendo/topics/automation/automation_c.html), [Logic Pro automation overview](https://support.apple.com/guide/logicpro/automation-overview-lgcpb1a1ea03/mac) |
...
3. Track管理のうちinstrument / drum追加、non-master整理、一般Track改名・削除、内蔵synth音色選択を部分完了する。名前依存の学習Trackは改名・削除を保護し、Audio / Busは後続基盤へ送る。
4. Project schema v3 として Track semantic role、Audio Asset、Automation、Tempo/Signature Map を導入する。
5. schema v3 を使う Audio Clip 配置・再生・非破壊 trim/gain/fade を実装する。
...
- renameはlocal draftから1 commitにまとめる。ただしschema v2で役割が名前依存のChords / Bass / Melodyは改名・削除を保護し、role永続化まで「全Track改名・削除対応」と数えない。
...
- 残作業は、Batch 4のsemantic role永続化と学習Trackのrole-aware管理、Batch 5のAudio Track配置・再生、Batch 6のBus routingである。

### Batch 4: schema v3 — Track Role / Audio Asset / Automation / Tempo Map
...
- `AudioAsset` は application-owned file と metadata/checksum を持ち、Audio Clip は asset ID と非破壊 source range を参照する。
- Automation は target、point、interpolation を持ち、最初は volume/pan を対象にする。
- Tempo/Signature Map は beat-domain の正本とし、単一の変換関数を live scheduler、offline WAV、MIDI、timeline が共有する。
- SQLite snapshot と assets directory の commit/recovery順を仕様化し、missing/orphan asset を検出する。

### Batch 5: Audio Clip 配置 / 再生 / 非破壊編集

- Batch 4の`AudioAsset`を参照するAudio Track / Clipをproduction UIから追加し、配置、移動、trim、gain、fade、loopを非破壊に保存する。
...
- [x] Batch 3 production導線（部分実装）: instrument / drum追加、non-master duplicate/reorder、一般Track delete/rename、学習Trackの名称・削除保護、canonical synth selectorを提供し、Audio / Busを未提供と表示する。
...
- [ ] Batch 3後続境界: Batch 4のsemantic role、Batch 5のAudio Track、Batch 6のBus Trackが完了するまで全Track CRUDと表現しない。
- [ ] Batch 4: v2→v3 migration が既存 fixture を失わず、Track role/Audio Asset/Automation/Tempo Map の exact roundtrip と壊れた入力拒否を満たす。
- [ ] Batch 4: variable tempo でも live/WAV/MIDI/timeline の拍位置が共通許容誤差内で一致する。
