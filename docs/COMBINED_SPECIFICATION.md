...
| Automation | Track音量/パンとMaster出力音量の時間変化 | Yes | non-Master Trackの音量 / パンとeffective Masterの出力音量について、beat snap付きlane editor、hold / linear、lane Bypass、Global / TrackまたはMaster Readを保存し、再生中はRead / Touch / Latch / Writeで対応操作を記録する。mode自体はruntime-only |
...
| Elastic Audio | Audio Clipのタイミング / 単音pitch補正 | Partial | 60秒以内のready・非loop Clipでmanual timing point、単音pitch region、補正量、`formantMode: off / preserve`を非破壊編集し、解析波形 / pitch trace、A/B、Undo / Redo、保存、live / full / selected WAVへ接続する。manual formant / vibrato編集、polyphonic、極端なrate / shift品質、take folder / loop-cycle、phase-coherent multitrack、自動クオンタイズ / grooveは未実装 |
...
- transport loopがOFFなら従来のone-shot録音、ONなら開始前に曲内の明示`[loopStartBeat, loopEndBeat)`と2〜128の固定pass数を凍結する。cycleはloop左端から1本の連続local captureを行い、同じ将来frameから有限の伴奏再生を開始し、Nth右境界で伴奏だけを止める。正の録音latency tailまで含めたeffective captureは0.5〜60秒とし、各passを累積丸めしたexact frame境界で同じ長さのAssetへ分割する
...
- 実測校正は通常の録音wizardとは分け、オーディオinterfaceの出力を選択入力へケーブル接続する外部I/O校正として案内する。スピーカーからマイクへの空中loopbackは禁止し、app monitorに加えてinterface / driver mixerのDirect Monitor・hardware Loopback・同一outputへのreturnもOFFにするよう開始前に明示する。開始時はtransport表示がstoppedでも保持中のnatural drain session graphを同期disposeしてからprobeを準備する。probe振幅は固定の低levelとし、排他中だけapp-wide AudioContextのengine output/calibration gainを既知unityへ正規化してfinallyで元値へ戻す。固定PRBSをengine output / limiter経由で複数burst送出し、同じ将来render frameから選択入力をcaptureする。各burstは最大500 msの整数sample lagを正規化相関し、silence、clipping、同率または近接peakによる曖昧さ、低confidence、context generation / sample rate変化をfail closedにする
...
### 7.10 Audio Clipのタイミング / 単音pitch補正（schema v10、部分実装）

- ready・非loop・canonical 48 kHz mono/stereo PCM16 WAVを参照する60秒以内の直接Audio Clipだけを採用対象にする。loop / cycle、missing / changed / unavailable asset、録音中、保存処理中、Project切替中はcontrolを無効にして同じpanelで理由を表示する。linked Clip / take folderや60秒超などdomain対象外の候補は解析または確定前に説明付きで拒否する
- Audio Clip Editorの「音声を整える」disclosure内に「タイミング」と「単音ピッチ」の2 tabを置く。未編集Clipはsource windowの始点 / 終点だけを持つlinearな初期状態から始め、始点 / 終点は移動・削除できない
- タイミングpointはsource frameとClip先頭からのtarget beat offsetを対応付ける。pointer、左右キー、label付き数値inputで移動し、追加・削除・resetを行う。source / targetの隣接区間は40 ms以上、local stretchは0.5〜2倍、全体で2〜128 pointとし、順序逆転や上限超過を候補だけ拒否する
- 「音程を解析」は元のcanonical Clip windowを端末内で解析し、bounded waveform、pitch trace、半音guide、最大128件の単音候補を表示する。解析候補の削除はProjectを変えず、pitch / 補正量の変更、split / merge、確定で初めて`pitchRegions`を1 gesture = 1 Undoとして保存する
- pitch regionは重ならないsource frame範囲、source / target pitch（MIDI cents）、0〜100%の補正量、境界transitionを持つ。実効shiftは±300 cent以内に制限し、scaleへ寄せる操作も最終的には同じregion metadataへ確定する
- 「音程を変えても音色を保つ」checkboxはnative accessible nameそのままで、保存済み`formantMode`を`off`と`preserve`の間で切り替える。候補がない時とbusy時はdisabledにし、accepted切替は各1 Undo / save revisionとしてlive、全体WAV、選択Track WAVの共通request / cache keyへ反映する。A/Bはこのmodeを変更しない
- accepted editはactive playbackを止めて有限なplayheadを保持する。次のlive再生、全体WAV、選択Track WAVは同じversioned WSOLA派生PCMを使い、source range / gain / fade / routing / automationは従来経路を維持する
- 「ピッチ補正前 / 補正後」のA/Bは選択Clipの`pitchEnabled`だけをlive用immutable snapshotで一時的に外す。タイミング補正は両方で維持し、現在位置から通常のlive playback requestを再開する。Project / history / revision / autosave / SQLite / `.ctsproj.json`とWAV出力は変更しない
- trim / splitではsource frameを基準にmarker / regionをcrop・rebaseして残す。audioWarp付きClipのloop化、take folder化、Auto Punch対象化は対応済みと見せず、domain境界で拒否する
- manual formant editing、vibrato編集、polyphonic pitch、take folder / loop-cycleへのwarp、phase-coherent multitrack stretch、audio quantize / groove抽出、audio follow / Smart Tempoは未実装である。off / preserveは合成fixtureでのみ品質gate済みで、実声のライセンス済みブラインドA/BまたはMUSHRA、unsupported rate / extreme shiftの品質保証は未達である
...
ライブのTrack出力とWAV PCMは同じstable routing DAGとProject Master gainをlimiter直前で一度だけ適用する。ライブは`session output → session Master fader → engine output/calibration gain → limiter`とし、Project scalar / automation / gestureはsession Masterだけを所有してengine gainをunityに保つ。Bus soloは関係する上流・下流edgeだけを開き、上流sourceの無関係なMaster直通edgeを漏らさない。ライブ専用のメトロノームもMaster faderを通し、ライブのMaster meterはpost-fader信号を表示する。WAVにはメトロノームclickとUI meter / analyserを含めない。Trackのmute / solo、各Track volume、Master volumeは、再生開始時およびoffline renderではsample 0から確定値を使い、再生中に値を変更した場合だけ10msで平滑化する。

schema v3で導入したAutomationLaneはnon-Master Trackのvolume / panを対象とし、schema v8ではcanonical Track順の最初のeffective Masterだけにoutput volume targetを追加する。Master panとlater Master targetは拒否する。Track scalarを最初のpointまでのbase valueとしてhold / linear補間をライブとWAVへ同じbeat→time変換で適用する。schema v6の必須`bypassed`とschema v7の必須`automationReadState`を共通resolverへ入力し、effective Readを`globalEnabled && !disabledTrackIds.includes(trackId) && !lane.bypassed`で決める。falseならpointを保持したままcommandを生成せず、そのtargetのTrack scalarを曲本体からrelease / effect tailまで使う。各pointの`interpolation`はそのpointから次のpointまでの出力方向の意味を持ち、`hold`は現在値を保ち、`linear`は次の値までbeat上で直線変化する。

Editorの「オートメーション」tabは選択中の対応Trackへ結び付き、Global Read、選択TrackのTrackまたはMaster Read、Track別のRead / Touch / Latch / Write、対応target、beat snapをnative controlで操作できる。non-Masterは音量 / パン、effective Masterは「Master出力音量」だけを表示し、later Masterには未対応を明示する。Global / TrackまたはMaster Readはlane Bypassと独立し、current schema v10へ保存する。laneがあればRead / Bypassを切り替え、Bypass中もcurveとpointを消さず編集できる。lane上または現在の再生位置へのpoint追加、選択pointのbeat / value / 次のpointまでの変化方法、1件削除、確認後のlane全消去を行える。Inspectorは実際に変更したfieldだけをpatchし、beat snapはbeat自体を確定した時だけ適用するため、無編集blurと値だけの編集ではimport済みoff-grid beatの精度を変えない。音量は0〜2、パンは-1〜1、beatは0〜曲末で、同じlaneの同beat重複を採用しない。laneがなければ最初の追加時にRead状態で作り、最後のpoint削除または全消去ではlane自体を除去する。追加・確定編集・削除・全消去・各Read gate切替はそれぞれ1 gestureをProject変更1回、Undo 1回、自動保存revision 1回として採用し、no-opまたは拒否では履歴を増やさない。保存・再読込後も独立laneとpoint ID、beat、value、interpolation、bypassed、Global / TrackまたはMaster Readを保つ。

AutomationLaneは再生session snapshotであり、lane編集またはRead / Bypass切替はactive playbackを停止して有限なplayheadを保持し、natural drainも破棄して次のplayで再構築する。laneが1件以上ある状態でのmixer / effect編集も原則同じ扱いだが、active write pass中に開始時Readだった別Trackのvolume / pan scalarだけを変える場合はpassをexact rebaseする。対象にeffective Read laneがなければAudioParamだけを更新して再生を維持し、laneがあれば最初のpoint前や次loopでscalarが可聴になるため、古い予約値を残さず再生を停止して次回再構築する。改名・ノート編集などmixerに無関係なProject変更では、予約済みAudioParam automationをcancelしない。transport loopと可変tempoでも既存の共通resolverを使うため、ライブ再生とoffline WAVは同じRead / Bypass結果になる。

再生開始時に対応Trackごとのmodeを凍結し、Read以外が1件でもあればruntime-onlyの1 passを開始する。Touchはミキサー / Track Listの対応controlをpointerまたはkeyboardで操作している範囲だけを書き、releaseから100 msで開始前curveへ戻す。Latchは最初の接触からパンチアウトまで最後の操作値を保持する。Writeは確認dialogを通過した時だけ有効になり、接触がなくてもpass開始からnon-Masterの音量 / パン、effective Masterの出力音量だけを開始時scalarで所有して既存curveを置き換え、パンチアウト後はTouchへ戻す。記録は既存laneの`bypassed`とGlobal / TrackまたはMaster Readを暗黙変更しないため、Read gateが無効なまま確定したcurveは再有効化するまで可聴化されない。Track ListとMixerの両Master faderは再生中のRead曲線を表示し、Write所有中はscalar、Touch/Latchのgesture中は操作値を優先する。

pass中のsampleはProject / history / revisionを変えず、停止、自然終了、seek、transport loop右端、mode / Read / Bypass変更、Undo / Redo、Project切替、native close、pagehideの境界で半開区間をパンチアウトする。確定候補は開始時Project参照とcanonical fingerprintへのCAS、point / lane上限、全Project codecを通過した時だけProject変更・Undo・save revision各1回として採用し、失敗時は部分curveを公開しない。pagehideで確定できない場合はProject no-opでpassを破棄する。確定curveはcurrent schema v10の通常laneとしてlive / full WAV / selected Track WAVの共通resolverへ渡るため、記録時のmode名はoffline bounceへ影響しない。

Read / Touch / Latch / Writeの選択、Armed / Writing、gestureと再生passの所有権はrenderer runtime-onlyで、Project / history / SQLite / `.ctsproj.json` / OpenAPIへ保存しない。Project activationと再読込では全TrackをReadへ戻す。Master pan、later Master、insert / send / tempo parameter、MIDI CC / LFO modulation、Trim / Relative / Cross-Over / Fill、parameter group単位のSuspend、複数loopへ連続記録するpass管理は未実装であり、対応済みと表示しない。
...
- 新しい再生、手動停止、Project切替、AudioContextの`suspended / interrupted / closed`は残っているdrainを即時に破棄する。破棄時はsession outputとsession Masterを一度だけ切断し、app-wide engine output/calibration gainへfadeやProject scalarを残さない。transport loopのwrapではdrainを開始しない
...
- 入力は単音のマイク録音または録音済みfileを対象とする。表示と編集はMIDI化前のtransient候補だけに作用し、元音声を破壊編集しない。これはAudio Clipへ保存するElastic Audioとは別機能で、解析した鼻歌自体を修復しない。polyphonic transcription、歌詞認識、formant / vibrato編集は未対応としてUIとgap matrixに明示する
...
- non-Master Trackと最初のeffective Masterではlaneの有無にかかわらず、Global Readと選択TrackのTrackまたはMaster Readを独立した最低44pxのnative buttonで表示する。`aria-pressed`、オン / オフの文字、両方とlane Bypassから決まるeffective Readの説明を持たせ、GlobalまたはTrack / Masterを切ってもlane Bypassやpointを暗黙変更しない。これら2状態はProjectへ保存される。later Masterにはautomation未対応を明示し、操作controlを出さない
- Read / Touch / Latch / WriteはTrack名を含むnative radiogroupにし、ArmedとWritingを文字とlive regionで区別する。Touchは操作中だけ記録して100 msで元curveへ戻ること、Latchは最初の操作後も停止・seekなどのパンチアウトまで最後の値を書き続けることを初心者向けに説明する。non-Masterの音量 / パンとeffective Masterの出力音量range controlはpointer、矢印、Home / End、PageUp / PageDownのどの調整gestureでも開始・更新・終了をexactly once扱いにする。Track ListとMixerの両Master faderは同じRead曲線またはgesture値を表示する
- Write選択時は即座にmodeを変えず、modal dialogでnon-Masterなら「触れなくても音量とパンの両方」、effective Masterなら「触れなくてもMaster出力音量」を再生位置から置き換え、パンチアウト後はTouchへ戻ると警告する。取消はWrite controlへfocusを戻して以前のmodeを保ち、確認だけがWriteをArmedにする。pass確定後はTouchを選択状態にし、確定curveだけが保存されることをstatusで伝える
...
- mode、Armed / Writing、gesture所有権はruntime-onlyであり、保存 / 再読込後はReadへ戻る。Master pan、later Master、insert / send / tempo automation、MIDI CC / LFO、Trim / Relative / Cross-Over / Fill、parameter group単位のSuspend、複数loop passを利用可能に見せるcontrolや予約状態名は表示しない
...
- Audio Clipの`audioWarp` timing point / pitch regionに対するimmutable add / move / split / merge / crop / rebase mutation
...
decode / analysis失敗、cancel、0件、512件超、mapping / commit拒否ではProject fingerprintを変えない。current schema v10ではcompiled tempo mapの共通seconds↔beat resolverを使い、beat 0だけの固定mapも同じ経路で従来と同じ結果にする。
...
6. playback schedule、AutomationLane、TrackGraphはsession開始時snapshotであるため、採用されたTrack / Clip構造、semantic role、tempo / 拍子map、曲長、preset、automation lane、routingのsource / target / position / edge集合変更をcommit境界で停止し、有限な現在playheadを保持する。sendのgain / enabledだけは既存edge gainを10ms平滑更新する。AutomationLaneが1件以上ある時のmixer / effect変更も原則停止する。live write pass中に開始時Readだった別Trackのvolume / pan scalarだけを変える場合はpassをexact rebaseし、effective Read laneがないtargetだけはAudioParamを更新してsessionを維持する。laneがあるtargetは最初のpoint前または次loopで古いscalarが再予約されるためsessionを停止し、次のplayで新snapshotからclock / topology / voice / AudioParam commandを再構築する。改名、mixerに無関係なevent編集、拒否、no-opでは予約済みautomationへ触れずsessionを停止しない

Track管理はschema v10 aggregate codecを正本にし、Undo/Redo、SQLite autosave、`.ctsproj.json`へrole、Audio metadata、Audio Clipの`audioWarp`（`formantMode`を含む）、take folder、automation bypass、Global / TrackまたはMaster Read、routingを含めて保存する。Audio Trackはproduction追加・再生・非破壊編集、stereo Busはproduction追加・routing・insert処理へ接続済みである。

### 5.8 Project schema v10 aggregate boundary

- current schemaは10。v6→v7は`automationReadState: { globalEnabled: true, disabledTrackIds: [] }`だけを加え、v7→v8は`schemaVersion`だけを更新する。v7では不正だったMaster automationを移行時に許容せず、canonical v8から最初のeffective Masterのoutput volumeだけを新たに表現できる。v8→v9は既存Clipを変えずoptionalな`audioWarp` timing / monophonic pitch metadataを追加し、v9→v10は既存`audioWarp`へ`formantMode: 'off'`を追加して既存の可聴結果を変えない。canonical v10は`off | preserve`を必須とする。
...
- AutomationLaneはnon-Master Trackのvolume / panと、canonical Track順の最初のeffective Masterのoutput volumeだけを対象にする。Master panとlater MasterはTypeScript / Rust両境界で拒否する。`bypassed: false`ではpoint前をTrack scalar、point間をhold / linearで評価し、`bypassed: true`ではpointを保持したままcommandを0件にしてTrack scalarを使う。lookahead windowはhalf-openを保つが、曲末の最終windowだけexact end pointを含め、release / insert tailへ終値を維持する。beat-linearなlinear区間はtempo change beatごとに補間値commandへ分割し、seconds-linearなWeb Audio `linearRampToValueAtTime`と同じ曲線にする。同じbeatのlane point / tempo change / window endは1 commandへ決定的にまとめ、loop境界だけは前cycleの終値と次cycleのhold resetをこの順で残す。ライブ、full WAV、selected Track WAVは同じcommand plannerとProject snapshotのtempo change列を使う。
...
- `ready` AudioAssetはchecksumとdecode前metadata、Audio Clipはasset IDとframe単位のsource range / fade / gain、必要な時だけ`audioWarp`を持つ。AutomationLaneはnon-Master Trackのvolume / panまたはeffective Masterのoutput volume target、point、hold / linear補間、bypassedを持ち、同じcommand resolverをlive schedulerとfull / selected Track WAVへ接続する。
- TypeScript codecとRust native persistence境界はcanonical schema v10 JSONを同じrequired-field / unknown-field / routing DAG / take-comp / automation target / Read / audioWarp相関条件で受理・migrationする。binaryと派生PCMはJSONへ埋め込まず、AudioAsset checksum / byte lengthで別のcontent-addressed repositoryを参照する。

schema v10 aggregateとAudio Asset repositoryは正本を分離する。Project JSONは編集意味、Audio Clipのtiming / pitch metadataと`formantMode`、take comp、automation bypass、Global / TrackまたはMaster Read、確定済みautomation curve、routing、content identityを保持し、repositoryはimmutable binaryを保持する。解析候補、解析waveform / pitch frame、派生PCM cache、A/B状態はruntime-onlyである。Audio Clipのproduction配置・再生・trim / gain / fade / loop / split / duplicate / delete、初期Elastic Audioとoff / preserve、既存Clipからのtake grouping / comp範囲編集は実装済みである。manual formant / vibrato編集、polyphonic pitch、take / loop warp、phase-coherent multitrack、audio quantize / groove、Master pan、later Master、insert / send / tempo automation、modulationと高度write modeは後続である。
...
2. project-modelのpublic mutationはsource Projectをcanonical codecで先に検査し、non-Master volume / panまたはeffective Master volume target、曲内beat、target別value範囲、`hold | linear`、同一lane内のbeat衝突、lane / point上限をno-throwのtyped resultで検査する。最初のpointで`bypassed: false`のtarget laneを遅延作成し、全Project entityと衝突しないlane / point IDを同じ予約集合から発行する。更新後はbeat昇順へ並べ、最後のpoint削除またはclearでは空laneを除去する。
3. `setAutomationLaneBypassed`はlane IDとbooleanだけを受け取り、target / point / ID / Track scalarを保持して`bypassed`だけをimmutableに変更する。同値は元Project参照のsemantic no-op、missing laneと非booleanはtyped failureである。Bypass中のpoint add / update / removeは状態を暗黙にReadへ戻さない。
4. 候補Project全体もcanonical codecを通過した場合だけStudio actionが開始時のProject参照へcompare-and-swapする。成功したadd / update / remove / clear / Read-Bypass切替は各1 Project change、Undo 1回、save revision 1回で、semantic no-opは元Project参照を返し、stale snapshot、busy operation、invalid source / candidateではProject、history、revision、selection、transportを変更しない。
5. 採用されたlane編集またはRead / Bypass切替はsession snapshotを無効化するため、active playbackを1回停止して有限なplayheadを保つ。accepted切替は既存runtime stop境界を呼んでnatural drainも破棄し、次のplayback requestが新Projectからautomation commandを再構築する。no-opと拒否ではactive / draining sessionへ触れない。
6. snapは入力正規化でありschema fieldを追加しない。schema v3で導入したtarget、point ID、beat、value、interpolation、schema v6の`bypassed`、schema v7の`automationReadState`をcurrent SQLite autosaveと`.ctsproj.json`でexact roundtripする。live lookahead、transport loop、可変tempo分割、offline WAVは`globalEnabled && !disabledTrackIds.includes(trackId) && !lane.bypassed`を判定する同じresolverを使い、effective Readがfalseなら全windowで空command列を返すため、Editor固有の可聴判定を作らない。
7. Trackまたはeffective Master / Global Readのproduction controlは同じ永続stateを変更するが、live write-mode captureは次節のruntime transactionで扱う。Master pan、later Master、insert / send / tempo parameter、MIDI CC / LFO modulationは対象外とする。

#### 5.8.2 Automation Read / Touch / Latch / Write runtime transaction

1. Storeはautomation対応Trackごとの`AutomationWriteMode = read | touch | latch | write`、Armed / Writing、touch中target、pass ownershipをrenderer runtimeだけに保持する。Project activationは全TrackをReadで初期化し、modeをProject codec、history、SQLite、`.ctsproj.json`、OpenAPIへ投影しない。
2. 再生graphのattach時に1件でもRead以外の対応Trackがあれば、project-modelの`beginAutomationPass`へnon-Masterとeffective Masterのmode、有限な開始beat、exact Project参照を渡す。passは開始時Projectのcanonical fingerprintとdeep-frozen snapshotを所有し、live操作中はProject / history / revisionを変更しない。音声録音中、stale activation / playback、無効Track / modeではgraph override前にfail closedする。
3. Mixer / Track Listのnon-Master volume / panとeffective Master volumeは共通gesture adapterを通す。pointerdown / move / up / cancel / lost captureと、矢印 / Home / End / PageUp / PageDownのkeydown / keyup / blurを同じpending identityへ正規化し、各targetのbegin / update / endをexactly once呼ぶ。live graphは対象AudioParamだけのscheduled curveをcancelしてmanual overrideを所有し、Touch releaseはtempo map上の100 ms後に開始前curveへ復帰、Latch / Writeはパンチアウトまでoverrideを維持する。両Master faderは同じruntime gesture値を購読し、片方の操作中も表示を一致させる。
4. Touchは接触区間だけを置換し、release後100 msのlinear return boundaryで開始前curveへ戻す。Latchは最初の接触からhalf-open punch-outまで最後のsampleを保持する。Writeはpass開始時にnon-Masterのvolume / pan両target、effective Masterのvolume targetを開始時Track scalarでcaptureし、接触がなくても対応curveを置換する。既存laneの`bypassed`、Global / TrackまたはMaster Read、対象外point object / IDは保持し、sampleはvolume 0.001、pan 0.0005の最大再構成誤差で決定的に削減する。
5. 停止、自然終了、seek、loop右端、mode / Read / Bypass変更、Undo / Redo、Project activation、native close、pagehideは同じpunch-out pathを使う。終了beatをProject範囲へclampし、source exact参照 / fingerprint、lane / point上限、fresh global ID、candidate codecを検証してから、開始時Projectへのstrict CASをProject変更・Undo・save revision各1回として採用する。commit拒否ではpassをretry可能なまま保持し、pagehideだけは同期確定できなければProject no-opでcancelする。
6. active pass中に許す並行Project変更は、開始時Readだった別Trackのvolume / pan scalarだけである。exact source参照とfingerprintに対する2段階rebaseでpassのfrozen baselineを維持する。effective Read laneがないtargetだけをlive graphへ反映し、laneがあるtargetはcommit後にsessionを停止して古いbase予約を破棄する。Write / Touch / Latch Trackのscalar、lane / routing / effect / noteなどとの混在候補はpassを失わずatomicに拒否する。
7. 確定curveは通常のschema v8 `AutomationLane`として次のlive再生とfull / selected Track WAVの共通resolverへ入り、記録時modeはbounce結果を変えない。成功したpunch-outまたは明示cancelと同じCoordinator transactionでWriteをTouchへfallbackし、React mount有無に依存させない。point / lane上限、CAS競合などで確定に失敗したpassは理由をUIへ公開し、明示cancelだけが部分curveを採用せずoverrideを解放してtransportを停止する。Master pan、later Master、insert / send / tempo、MIDI CC / LFO、Trim / Relative / Cross-Over / Fill、parameter group単位のSuspend、複数loopへ連続記録するpass managementは別gateとする。

#### 5.8.3 Tempo / 拍子map editing transaction
...
5. 採用されたmap変更は再生session snapshotを停止し、有限なplayheadを保持する。次のplayback / metronome / WAV / MIDI / timelineは既存のcompiled musical-time indexを再構築するため、Editor固有の変換器を作らない。map entity自体はschema v4から変えず、current schema v10へそのまま保存する。
...
8. permission / device-ended / context世代変更 / clock不連続 / capture arm / cancel / canonicalize / store失敗、stale snapshot、target消失、revoked tokenではProject / history / revision / selectionを変更しない。保存後CAS拒否のorphan bytesは許容するが、欠損bytesを参照するmetadataを作らない。既存Audio Clipのtake grouping / comp編集、固定pass cycle、bounded Auto Punchは別transactionで提供する。再生中へ途中参加するQuick Punch、長時間streaming、複数入力は後続境界である。
...
2. 校正開始はrecording / playbackと排他にし、transportが既にstoppedでも`PlaybackController`のnatural drainを同期stopしてsession output / Master graphを破棄してから、app-wide engine output/calibration gainを校正専用に所有する。`AudioContext`のgeneration / sample rateと選択中のexact input IDをimmutableなrequestへ凍結する。固定seedから生成した既知PRBSを無音で区切った複数burstとしてengine output / limiter経由で流し、capture Workletを同じ将来render frameへarmする。通常のTrack、Clip、AudioAsset、Project mutationや録音asset保存経路は作らない。
...
6. 実測modeの録音配置はprofileの`latencyFrames / sampleRate`を唯一の自動補正値とし、hostのinput track latency、`baseLatency`、`outputLatency`、limiter look-aheadを重ねない。runtime-only manual offsetは実測値の後にだけ加える。物理校正自体は長時間streaming、Quick Punch、複数I/O routingを実装済みにしない。
...
7. このincrementは全PCMをRAMに保持するbounded local captureである。bounded Auto Punchは5.10.3の別transactionとし、長時間disk streaming、再生中へ途中参加するQuick Punch、複数同時入力、MIDI cycle comp、named comp、flatten / bounceは含まない。

### 5.10.3 Bounded non-destructive Auto Punch

1. Storeはloopとは独立したruntime-onlyの`punchEnabled / punchInBeat / punchOutBeat / punchPreRollBeats / punchPostRollBeats`を持つ。locatorはProject codec、history、SQLite、`.ctsproj.json`へ投影せず、Project activationで初期化する。loopとPunchは同時に有効化せず、切替時も双方の範囲値を保持する。編集はtransport停止・Project idle・recording token未所有時だけ受理する。
2. 開始前にexact armed Audio Trackと`[punchInBeat, punchOutBeat)`を所有handleへ凍結し、pure `inspectAudioPunchTarget`をpermission前と採用前に実行する。空き窓、範囲全体を覆うready non-loop Clip 1件、同一窓のtake folder 1件だけを受理し、部分 / 複数重複、loop、unavailable source、異なるfolder窓、source / entity上限不足をtyped failureにする。
3. pre-roll開始beatとpost-roll終了beatを曲端へclampし、compiled variable-tempo mapの同じplayback anchorからpunch-in、punch-out、playback endを累積秒→safe integer frameへ1回だけ丸める。Workletはplayback anchorではなく`anchorContextFrame + captureStartOffsetFrames`へarmする。正latencyは`punchFrameCount + latencyFrames`だけcaptureして先頭をtrimし、負latencyはexact output先頭へsilenceを置く。canonical resample後も期待frame数との差を1 frame以内に制限する。
4. Schedulerはpre-rollから有限sessionを開始し、対象Trackのroute audibility gateだけをexact AudioContext timeの半開`[in, out)`で0へする。対象Trackの前後と他Trackは元のmute / solo / routing解決を保つ。post-roll終端callbackはexact request IDをrecording ownershipへ一度だけ刻み、UI coordinatorはcaptureとpost-rollのどちらが先でも両方が揃うまでfinalizeしない。manual stop / interruptionは証明にならない。
5. exact PCMをcanonical Assetとしてcontent-addressed repositoryへ先に保存する。空き窓はexact Clipを追加する。spanning Clipは左右のsource window / 外端fade / gainを保持し、旧中央をTake 1、新録音をTake 2、Take 2全域のComp 1件へatomic変換する。exact folderは新takeを末尾追加して全域採用する。Storeは候補を同じdomain mutation・exact entity ID・timestampで再実行し、codec JSONが完全一致する場合だけ1 CAS / 1 history entry / 1 Undoで公開する。
6. cancel、unmount、device / context / clock変化、punch-out前またはpost-roll端前のtransport stop、late / duplicate callback、canonicalize / store failure、stale Project / operation / request、domain replay不一致ではProject / history / selectionを変更しない。自然post-rollがcapture tailより先でもexact request証明を保持し、capture失敗時は採用しない。
7. このincrementは最大60秒・単一入力・RAM boundedで、開始時に既存再生を停止してpre-rollから新規sessionを作るAuto Punchである。再生中のsessionへ途中参加するQuick Punch、automatic input monitoring、cycle＋Punch、長時間disk streaming、任意重複の自動修復、複数入力、MIDI comp、named comp / flattenは含まない。
...
5. accepted topology / comp選択変更はactive playback sessionを停止して有限なplayheadを保持する。選択folder / take、pointer preview、Inspector draft、focusはruntime-onlyである。reload後はv5で導入されcurrent schema v10へ継承されたfolderを復元し、次のplayとWAVが同じpure `AudioClipPlaybackIndex`へ正規化する。
...
8. 固定pass cycle recordingとbounded Auto Punchから生成したfolderも同じ編集 / plannerを使うが、そのcapture / atomic生成は5.10.2 / 5.10.3が所有する。再生中の任意Quick Punch、MIDI take / comp、複数入力、複数の名前付きcomp、flatten / bounceはこのtransactionの完了条件に含めない。

### 5.12 Elastic Audio timing / monophonic pitch transaction

1. UIはready・非loopの直接Audio Clipと開始時Project / activationを凍結する。解析はAudioAsset resolverからchecksum検証済みのexact Clip windowだけを取得し、bounded waveform / pitch frame / 最大128 region候補をtransient resultとして返す。解析中のcancel、保存 / 録音busy、Project / Clip / Asset / activation変更、unmountではgenerationを進め、late resultをProjectへ採用しない。
2. project-modelは`AudioWarp`をimmutableに編集する。timing markerはsource frame / target beatとも厳密昇順、始終点固定、隣接40 ms以上、0.5〜2倍、2〜128件とする。pitch regionはsource window内で昇順・非重複、最大128件、実効shift ±300 cent、補正量0〜1、transitionはregion半分以下とする。trim / splitはsource frame基準でcrop / partitionし、target beatを新Clipへrebaseする。
3. accepted add / move / remove / reset / split / merge / pitch updateはcandidate全体のcanonical schema v10 validationと開始時ProjectへのCASを通過した時だけ1 Project change / 1 Undo / 1 save revisionへ採用する。解析候補の選択・除外、zoom、cursor、pointer preview、focus、A/B状態はProjectへ入れない。semantic no-op、invalid、busy、staleではProject / history / revision / transportを変えない。
4. accepted editはactive / draining playbackを停止し、有限なplayheadを保持する。次の再生 / WAV snapshotは同じ`compileAudioWarpRenderRequestIndex`からsource window、tempo map、target sample rate、timing knot、pitch region、保存済み`formantMode`を一度だけ整数frameへcompileし、algorithm version `wsola-v1/dsp-2`とAudioAsset checksumを含むcontent-addressed keyを作る。
5. A/Bは選択Clipの`pitchEnabled`だけをfalseにしたlive用immutable Project snapshotを作り、timing markerと他のClipを保持する。live中の切替はposition beatを保持して通常request generationを1回進める。Project / history / revision / autosave / SQLite / `.ctsproj.json`、全体 / 選択Track WAV snapshotは変更しない。
6. audioWarp付きClipのloop化、take folder化、Auto Punch spanning Clip化はdomain境界で拒否する。manual formant / vibrato編集、polyphonic pitch、take folder / loop-cycle、phase-coherent multitrack、audio quantize / groove、audio follow / Smart Tempoはこのtransactionに含めない。
...
- `AudioAssetPlaybackCache`はcurrent schema v10の`ready` assetだけを受理し、repository readの前後で128 MiB objectのlength / SHA-256を照合する。再生対象のraw preflightとdecoded AudioBuffer cacheは各256 MiBを上限とし、decode cache keyへchecksumとtarget AudioContext sample rateを含める
...
- ライブ再生はcurrent schema v10のcompiled routing DAGに従うTrack / Busのmain outputとpre/post-fader sendを`session output → session Master fader → engine output/calibration gain → limiter`へ接続する。Project scalar / Read automation / write gestureはsession Master faderだけを所有し、engine gainは通常unityに保つ。ライブ専用メトロノームもsession outputへ加え、session Master fader直後へpost-fader UI meterを接続する。OfflineAudioContextは同じrouting planとMaster gain resolverをrender-owned Master fader→limiterへ接続し、メトロノームclickとUI meter / analyserを作らず、full / selected Track WAVで別経路のgainを重ねない
- graph初期化時とoffline renderでは、Trackのmute / solo、各Track volumeおよび有効Master volumeをsample 0から確定gainへ設定し、既定gain 1.0の漏れを許さない。10ms平滑化は、再生中にTrack volume / mute / soloまたは有効Master volumeを更新した時だけ使う。有効なMaster Read lane中のbase scalar変更は現在beatを保持したsession replacementとして再開し、古い予約値を残さない
...
- Master UI analyserとmeter registry entryはaccepted playback sessionとそのsession Master faderが所有する。構築途中の失敗、session replacement、graph破棄では、そのsourceとanalyser identityがregistryの現在値に一致する場合だけcleanupし、古いsessionの後始末が新しいentryを削除しない
...
- drain末尾fadeはProject Master faderとengine output gainを上書きせず、session output gainだけをabsolute cleanup deadlineの6ms前までautomationする。callbackがfade endより遅れた場合はsession outputを即時0にし、cleanup deadlineより遅れた場合はtimerなしで完了する。いずれも現在時刻からtailを延長しない。新しい再生、手動停止、Project activation、context interruption、controller / bridge disposeはtimerとsession graphを即時破棄し、pending fadeをcancelする。engine output/calibration gainには触れない
...
### 6.1.7 Elastic Audio Worker / DSP / cache

- `compileAudioWarpRenderRequestIndex`はcurrent schema v10 Projectを先に検証し、編集された直接Audio Clipだけをcompileする。tempo map上のtarget beatをtarget sample rateの整数output frameへ1回だけ丸め、source windowのframe index、timing knot、実効pitch shift、保存済み`formantMode`、algorithm version `wsola-v1/dsp-2`、AudioAsset checksumから決定的なcache keyを作る。未編集Projectは従来のdecoded AudioBuffer経路をそのまま使う
- source resolverはcanonical 48 kHz mono/stereo PCM16 WAVのexact Clip windowをparseし、main threadからWorkerへchannel ArrayBufferをtransferする。Workerはpitch shift後にWSOLAでtiming knot間を0.5〜2倍へ伸縮し、requestどおりのsample rate / channel / exact frame数を持つfinite Float32 PCMだけを返す。protocol、shape、非有限sample、source window、上限違反は`invalid-request` / `invalid-pcm` / `resource-limit`として型付きに拒否する
- Worker clientは単調generationとAbortSignalを所有する。Project / Clip変更は旧generationを同期的に無効化してWorker queueへcancelを送り、CPU処理中のAbortはoperation専用Workerをterminateする。late / duplicate / unknown responseをcache、live graph、WAVへ採用せず、Worker errorではpending requestを全件settleしてProjectは保存済みmetadataを保持したまま再試行可能にする
- `AudioClipBufferCache`はcache key単位のsingle-flight、lease、leased entryを追い出さないLRUを持つ。派生PCMは1 entry / cache合計128 MiB以下で、algorithm version、checksum、source window、target rate、timing / pitch値、`formantMode`が変われば別keyになる。lease解放後だけLRU evictionし、未完了entryの全waiterがなくなればrenderをAbortする
- Worker input copy、source / pitch / WSOLA intermediate、derived outputのworst-caseをchecked arithmeticで見積もり、process-wideの重い音声処理384 MiB台帳へallocation前に予約する。派生cache保持中は同じ台帳のreservationをcache lifetimeへ移し、import / recording / humming / live / WAVとの同時実行も合算上限を越えない。上限超過はWorkerやOfflineAudioContextを作る前にfail closedする
- `formantMode: preserve`はsupported rateの安定した単音有声音を先に検査する。基音候補はchannel別の倍音振幅をenergy-linked RMSで集約し、奇数倍音と補間した偶数倍音の証拠で選ぶ。奇数倍音の証拠が実質ないmissing-fundamental入力だけ整数倍候補を許可し、弱い奇数倍音を持つ声は元の基音を維持する。時間frameごとの倍音包絡とchannel位相が定常かつ3共振modelのlog-RMSEが0.04以下の時だけ、移動後の倍音gridへ共振包絡を再合成する。疎な奇数倍音は偶数倍音から包絡をfitして奇偶の励振差を残し、pitch-downで追加する倍音も同じ奇偶の最終source倍音を参照する。無音、attack / decay、異母音、noise、vibrato、位相変化、model不一致は時間局所のlinked cepstral transfer、解析gate不成立はsample-exactなoff経路へfallbackする
- live、全体WAV、選択Track WAVは同じ`prepareAudioClipPlaybackBuffers`境界で必要assetをresolveし、同じrender request / cache leaseからAudioBufferを作って既存のAudio Clip plannerへ渡す。gain、fade、routing、automation、tailは派生PCMの後段で従来の共通経路を使う。full / selected scopeが同じClipを鳴らす場合はderived PCM16が一致し、A/Bのpitch bypassだけはlive用Project snapshotに限定する
- off / preserveは初期DSPの実装範囲である。manual formant editing、vibrato編集、polyphonic pitch、take folder / loop-cycle、phase-coherent multitrack、auto quantize / groove extraction、audio follow / Smart Tempo、unsupported rate / extreme shiftの品質保証は範囲外である。合成fixtureは共振再合成と安全fallbackを確認するが、実声品質はライセンス済み音声によるブラインドA/BまたはMUSHRAを別release gateとする
...
- Project schemaのcurrent versionは10。v1の`aliasOf`はruntime上の意味を持たず各Clip自身のpayloadが再生されていたため、v1→v2 migrationでは`aliasOf`だけを削除して独立Clipとして保持する。v2→v3はrole、tempo/signature map、AudioAsset / Automation metadataを決定的に加え、固定tempoの音とlegacy audio参照を失わない。v3→v4は全non-Masterへdirect-to-Master outputをexact 1件、sendを0件作る。v4→v5は空の`audioTakeFolders`、v5→v6は全AutomationLaneへ`bypassed: false`、v6→v7は`automationReadState: { globalEnabled: true, disabledTrackIds: [] }`だけを加えて既存の音を変えない。v7→v8は`schemaVersion`だけを更新し、v7で不正だったMaster targetを移行時に許容しない。v8→v9は既存Clipを変えずoptional `audioWarp`を許容し、v9→v10は既存warpを`formantMode: 'off'`へ移行する。TypeScript codecとRust native metadata境界は`v1 → v2 → v3 → v4 → v5 → v6 → v7 → v8 → v9 → v10`を順に適用し、canonical v10を保存する。
- schema v10では`lengthBeats`とtempo / 拍子mapが正本で、`bpm` / `timeSignature` / `lengthBars`は検証対象のcompatibility mirrorである。Track roleは名前から独立した正本、AudioAssetは`ready` / `unresolved`のmetadata union、Audio Clipはoptionalな`audioWarp`（存在する場合は必須`formantMode`を含む）、AudioTakeFolderはimmutable source windowとgapless comp、Automationはnon-Master volume / panまたはeffective Master output volume target、point / interpolation、bypassed、Global / TrackまたはMaster Read、AudioRoutingはoutput / sendの非循環graphを持つ。
...
11. canonical Track順の先頭Masterを選び、output volume lane、Master Read、Read / Touch / Latch / Writeを操作する。Track ListとMixerの両Master faderがRead曲線と片側のgesture値を同時表示し、full / selected Track WAV、保存 / 再読込でも同じcurveになることを確認する

期待結果:

- 追加・確定編集・削除・全消去は各1 gestureにつきUndo 1回、save revision 1回で、no-op、同beat衝突、範囲外、Master pan / later Master / missing Master targetはProject / history / playback / active write passを変更しない
...
- laneが存在するときだけRead / Bypass controlを表示し、Bypassはpoint / 編集状態を保持したままautomation commandを発行せずTrack scalarを使う。Readへ戻すと同じcurveが復帰し、各切替はUndo / save revision各1回、active sessionとnatural drainを停止する
- pointへkeyboardで到達・編集でき、削除後のfocusが回復する。320px幅ではdocument全体に横overflowがなく、lane時間軸だけが内部scrollする
- Global / TrackまたはMaster ReadとRead / Touch / Latch / Writeはproduction controlから操作できる。pass中はProject / history / revisionが不変で、停止 / 自然終了 / seek / loop右端 / mode・Read・Bypass変更 / Undo・Redo / Project切替 / closeの確定成功時だけ1 Undo / revisionを追加する。stale / resource / point上限 / CAS失敗では部分curveを採用しない
- Global / TrackまたはMaster / laneのRead gate、確定curve、Bypassだけが保存対象で、mode、Armed / Writing、gesture / pass所有権はProject JSONとOpenAPIに現れない。Master pan、later Master、insert / send / tempo automation、MIDI CC / LFO、高度write modeを実装済みと示すcontrolはない
...
- 6つのARIA tabとpanel関係、native control、削除後focus、44px target、timelineのみ内部scrollを満たす。固定pass Audio cycleとbounded Auto Punchから自動生成したfolderも同じEditorを使い、Quick Punch / automatic input monitoring / cycleとの併用 / MIDI comp / named comp / flattenを実装済みと示さない

### E2E-007: Audio Clipのタイミングと単音pitchを整える

1. ready・非loopのcanonical Audio Clipを選び、Audio Clip Editorの「音声を整える」を開く
2. タイミングpointを追加し、pointer / keyboard / 数値inputで移動して保存を待つ。始終点の移動 / 削除と狭すぎるpointを試す
3. 「単音ピッチ」で解析を開始し、cancelと再解析を行う。実waveform / pitch trace / cursorを確認し、解析候補を1件外してProject未変更であることを確認する
4. pitch regionのtarget / 補正量を変更し、split / merge / 削除、Undo / Redoを行う
5. live再生中に「ピッチ補正前」「補正後」を切り替え、位置とタイミングが同じままpitchだけが変わること、save表示が変わらないことを確認する
6. 元のselected Track WAV、編集後のfull WAV、編集後のselected Track WAVを各2回書き出し、PCMを独立parserで比較する
7. 保存・再読込後にmarker / regionを確認し、1024×640と320×800でkeyboard-only操作する
...
| Automation metadata / playback / write pass | non-Master volume / panとeffective Master volume target、lane / point ID、必須boolean `bypassed`、必須`automationReadState`をroundtripし、Master pan、later / stale target、重複ID、不正順序を拒否する。Global / TrackまたはMaster / laneの2x2x2 precedenceを共通resolverで検査し、disabled時はlive / full / selected Track WAVともcommandが0件かつTrack scalar、再有効化後はexact curveを復元する。Read変更は1 CAS / 1 Undo / 1 save revisionでactive snapshotを1回停止する。Touchの接触区間＋100 ms return、Latchのfirst-touch→punch-out、Writeのpass全域non-Master volume / panまたはMaster volume、決定的sample削減、outside curve / bypass保持、point上限atomic rejectをpure domainで検査する。Storeはpass中Project不変、境界ごとのexact-once punch-out、1 commit / Undo / revision、stale / recording conflict / CAS failure、Read Track scalar exact rebaseを検査する。有効Master Read lane中のscalar変更は同beatでsessionを置換・再開し、unsupported Master mode操作はactive passを確定しない。確定curveのlive / WAV command parity、SQLite save / reload、runtime modeがcodec payloadへ混入しないことを確認する |
...
### 7.8 Production Automation lane / Read / write-mode regression gate

- domain mutationはadd / update / remove / clearをpublic APIだけから検証する。source / candidate codec、immutable入力、lane遅延作成 / 再利用 / 空時prune、point厳密昇順、global ID、volume 0〜2 / pan -1〜1、beat 0〜曲末、lane 2,048 / point 20,000境界、同beat衝突、throwing ID factoryを網羅する。effective Master volumeを受理し、Master pan、later / missing Masterをfailure / no-opとして元Project参照のまま拒否する
...
- 4つのARIA tab / panel、Track未選択、non-Master、effective Master、later Master、空lane、対応target切替、snap、lane位置 / playhead追加、point選択、beat / value / interpolation編集、Delete / Backspace、確認付き全消去、inline alert、polite statusをcomponent / E2Eで検査する。effective Masterはvolumeだけ、later Masterはunsupported表示だけにし、全消去は確認表示と取消ではProject / history / revision不変、確定だけ1変更、Undo 1回でexact復元、取消と確定後のfocus回復を検査する
...
- lane作成前はRead / Bypass controlを表示せず、作成後はnative controlをRead→Bypass→Readと切り替える。Bypass中もpointの表示・選択・編集・保存を維持し、live / WAV共通plannerはautomation commandを0件としてTrack scalarを使う。Read復帰後のcommand列とpointを切替前とexact比較する
- Read / Bypass actionは開始時Project参照へのexact CAS、recording / save fence、semantic no-op、missing laneを検査する。採用された切替だけがProject / history / revision各1回で、active playbackとnatural drainを有限playheadのまま停止する。no-op / stale / busy / missing laneはProject / history / save / transportを変えない
- Global / TrackまたはMaster Readはlane Bypassと独立したnative buttonで、各2状態とlane 2状態の全組合せをproduction UIから切り替える。各採用は1 CAS / Undo / save revision、SQLite保存・再読込後にexact復元し、disabled中のscalar編集は対象AudioParamだけを更新して他Track / routing / effect / active write passを変えない。effective Read laneがあるtargetのscalar編集はpassをrebaseした後に同じ有限beatでsessionを置換・再開し、first point前や次loopで凍結済みの旧baseを鳴らさない
- Read / Touch / Latch / Write radiogroup、Armed / Writing status、Write確認dialogの警告、取消時focus / mode保持、確認後のWrite、pass終了後Touch fallbackをcomponent / Chromiumで検査する。effective Master Writeはvolumeだけを所有する。Global / TrackまたはMaster / mode controlは44px以上、keyboard focus-visibleを持ち、320pxでは縦に折り返してdocument横overflowを出さない
- pointerup / cancel / lostpointercapture / blurと、矢印 / Home / End / PageUp / PageDownのkeydown / keyupを共通gesture adapterへ通し、begin retry、update、endをexactly onceにする。Touchはrelease時だけ100 ms return、Latch / Writeはrelease後もoverrideを保持し、unmount / pagehideはactive gestureを終了してからpass確定へ進む
...
- fixed-pass cycle gateは明示loopのhalf-open範囲、2 / 128 pass境界、可変tempoでの累積frame丸め、正 / 負 / 0 latency、0.5秒未満、正tail込み60秒超過を純粋plannerで検査する。有限Schedulerは各passへ同じProjectを展開し、Nth右境界で1回だけ終了し、通常loopのInfinity契約を変えない。Workletはeffective exact frameで止まり、各Assetが同じoutput frame数、隣接passに欠落・重複がないことをsample fixtureで比較する
- cycle transactionは全passのcanonical bytes / receiptが揃うまでProject不変とし、pass順2〜128 take、first take全体の初期comp、new / existing target、selection、1 Project change / 1 Undoを検査する。manual stop / cancel / unmount / transport中断 / permission / device / clock / canonicalize / N件目store / final CAS failureを各段階で注入し、部分folder / Track / history / selectionを公開しないこと、retryやlate callbackが二重commitしないことを確認する
...
- 3OS実deviceでpermission、システム既定 / 明示device選択、`devicechange` / device loss、Record Arm先への追記、disk full、monitor feedback、close、再起動再生を確認する。shared AudioContextの伴奏同期と推定 / 実測 / 手動latency補正を有線・Bluetoothを分けて聴感 / 波形比較し、host申告値がない環境も確認する。実測はinterfaceの物理cable loopbackを使い、interface / driver mixerのDirect Monitor、hardware Loopback、同一outputへのreturnをOFFにして電気的feedbackがないこと、Project Master fader 0 / 1 / 2でも固定probe levelと測定値が一致することを確認する。OSごとに既知frame shiftとの誤差、silence / clip / ambiguity拒否、再校正案内を記録する。固定pass cycleは2 / 中間 / 最大passで境界click・drift・最終tailを波形比較する。bounded Auto Punchは次節の独立gateで検証し、長時間disk streamingは未対応として残す
...
- macOS / Windows / Linuxのrelease bundleと実入力deviceでpre-rollから伴奏、punch-in exact frameからcapture、target-only gate、punch-out後のnatural post-roll、正 / 負 / 0 latency、3 adoption形、Undo / 再起動を波形比較する。disk full、device loss、window closeもProject不変を確認する。Quick Punch、automatic input monitoring、cycleとの併用、disk streaming、arbitrary overlaps、input hot switch、multi-input、MIDI comp、named comps、flattenはloss-boundaryとして別gateに残す
