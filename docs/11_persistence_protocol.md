...
### 2.1 Project schema v4 / Audio Asset boundary

- Project schemaのcurrent valueは4。rendererとRust native境界はcanonical JSONをversion判定し、`v1 → v2 → v3 → v4`を順にmigrationしてからrequired / unknown fieldとdomain不変条件を検証する。SQLiteの`user_version = 2`はrepository table schemaのversionであり、Project JSONの`schemaVersion = 4`とは別である。
...
上記のcommit/read-back/checksumはschema v4のAudioAsset metadataをProject JSONの一部として保護する。Tauriのcanonical save / crash draftは、SQLite transactionへ入る前に全`ready` assetのapplication-owned objectを実byte length / SHA-256まで再検証し、missing / changed objectを含むsnapshotを成功扱いにしない。Webではimport時にIndexedDB objectを先に保存・検証し、load / playback時にもruntime availabilityを再検査する。
...
- ブラウザ版JSONはcompact canonical payloadで保存・通常書出し・緊急書出し・再読込を同じ16MB上限に揃える。AudioAsset metadataはschema v4 JSON、実binaryはIndexedDBへ保存し、現行の`.ctsproj.json`には同梱しない。binaryを`assets/`へ同梱するportable project bundleは未実装の将来案である。単体JSONのimportは対応objectが同じrepositoryに存在しない限り現在Projectを置換しない。
...
- native repositoryはlegacy persistence migration v1〜v4を受理し、同一snapshot/projectでは完了済みの最高versionだけをlive authorityとして扱う。これはProject schemaとは別のSQLite移行protocol versionであり、現行v4はpayloadをProject schema `v1 → v2 → v3 → v4`のcanonical metadataへ変換する。protocol v3の完了markerがあってもv4 statusは別物としてexact raw archiveを再評価する。v4が未完了の間は完了済みv3をlive authorityとして維持し、v4のatomic完了後だけ切り替える。旧v3の未完了stagingもimmutableな監査証跡として保持するがauthorityにはしない。これにより旧projectのunsupported/migration診断や旧branch/headはlive判定から外れる一方、異なるsnapshotのsticky evidenceとexact raw archiveは保持される。未完了の上位versionは下位versionを置き換えず、未知の将来versionがrunまたはstagingに残るdatabaseは初期化・全操作ともmutation前にfail closedする。rendererとnativeはschema v4 metadataを検証する。
