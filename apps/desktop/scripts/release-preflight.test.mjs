...
const runtimeTestOnlyMarkers = [
  ...
  'CTS_NATIVE_TEST_PROJECT_BUNDLE_ROOT',
  'CTS_NATIVE_TEST_PROJECT_BUNDLE_OPEN_PATH',
  'CTS_NATIVE_TEST_PROJECT_BUNDLE_SAVE_PATH',
...
async function createReleaseSource(version = '1.2.3') {
  ...
  await mkdir(
    path.join(root, 'apps/desktop/src-tauri/permissions/autogenerated'),
    { recursive: true },
  );
  ...
  await Promise.all([
    ...
    copyFile(
      new URL(
        '../src-tauri/permissions/autogenerated/file_open_project_bundle.toml',
        import.meta.url,
      ),
      path.join(
        root,
        'apps/desktop/src-tauri/permissions/autogenerated/file_open_project_bundle.toml',
      ),
    ),
    copyFile(
      new URL(
        '../src-tauri/permissions/autogenerated/file_export_project_bundle.toml',
        import.meta.url,
      ),
      path.join(
        root,
        'apps/desktop/src-tauri/permissions/autogenerated/file_export_project_bundle.toml',
      ),
    ),
  ]);
  await writeFile(
...
test('requires portable bundle commands, generated permissions, and main capability grants', async () => {
  const missingBuildCommand = await createReleaseSource();
  const buildPath = path.join(missingBuildCommand, 'apps/desktop/src-tauri/build.rs');
  await writeFile(
    buildPath,
    (await readFile(buildPath, 'utf8')).replace('"file_open_project_bundle",', ''),
  );
  await assert.rejects(
    validateDesktopSecurityPolicy({ rootDir: missingBuildCommand }),
    /build manifest must register file_open_project_bundle/,
  );

  const missingInvoke = await createReleaseSource();
  const libPath = path.join(missingInvoke, 'apps/desktop/src-tauri/src/lib.rs');
  await writeFile(
    libPath,
    (await readFile(libPath, 'utf8')).replace(
      'native_files::file_export_project_bundle,',
      '',
    ),
  );
  await assert.rejects(
    validateDesktopSecurityPolicy({ rootDir: missingInvoke }),
    /invoke handler must register file_export_project_bundle/,
  );

  const invalidPermission = await createReleaseSource();
  ...
  await writeFile(
    permissionPath,
    (await readFile(permissionPath, 'utf8')).replace(
      'commands.allow = ["file_open_project_bundle"]',
      'commands.allow = ["file_open_project"]',
    ),
  );
  await assert.rejects(
    validateDesktopSecurityPolicy({ rootDir: invalidPermission }),
    /Generated permission must allow only file_open_project_bundle/,
  );

  const missingCapability = await createReleaseSource();
  ...
  capability.permissions = capability.permissions.filter(
    (permission) => permission !== 'allow-file-export-project-bundle',
  );
  await writeFile(capabilityPath, JSON.stringify(capability));
  await assert.rejects(
    validateDesktopSecurityPolicy({ rootDir: missingCapability }),
    /Tauri capability permissions.*allowlist/,
  );
});
...
test('rejects production build hooks, remote assets, platform overrides, and new Cargo authority', async () => {
  ...
  const portableDocsScriptRoot = await createReleaseSource();
  const portableDocsPackagePath = path.join(portableDocsScriptRoot, 'package.json');
  const portableDocsPackage = JSON.parse(
    await readFile(portableDocsPackagePath, 'utf8'),
  );
  portableDocsPackage.scripts['docs:portable-bundle:check'] =
    'node scripts/unreviewed-portable-docs-check.mjs';
  await writeFile(portableDocsPackagePath, JSON.stringify(portableDocsPackage));
  await assert.rejects(
    validateDesktopSecurityPolicy({ rootDir: portableDocsScriptRoot }),
    /Root production package identity.*allowlist/,
  );
