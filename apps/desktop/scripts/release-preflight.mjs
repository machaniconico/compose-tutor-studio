...
const productionRootPackageIdentity = Object.freeze({
  ...
  scripts: {
    ...
    'docs:portable-bundle:check': 'node scripts/check-portable-bundle-docs.mjs',
...
const productionLibraryPackageIdentities = Object.freeze({
  ...
  'packages/project-bundle/package.json': libraryPackageIdentity('@cts/project-bundle', {
    '@cts/project-model': 'workspace:*',
  }),
...
const productionBuildScriptSha256 =
  'c38904ca7aa2afdc95ba0de3bcb19c6d5e4554fb73fe9a82a8605215c4fc3ab0';
...
const productionPnpmLockSha256 =
  'c3387aa743db4caa383795a58368c23313949c3f0be6683f8f8bdc2f4666d84f';
...
export async function validateDesktopSecurityPolicy({
  ...
  const portableCommandContracts = [
    {
      command: 'file_open_project_bundle',
      permission: 'allow-file-open-project-bundle',
      file: 'file_open_project_bundle.toml',
    },
    {
      command: 'file_export_project_bundle',
      permission: 'allow-file-export-project-bundle',
      file: 'file_export_project_bundle.toml',
    },
  ];
  const [buildScriptSource, libSource, ...portablePermissionSources] = await Promise.all([
    readFile(path.join(tauriRoot, 'build.rs'), 'utf8'),
    readFile(path.join(tauriRoot, 'src/lib.rs'), 'utf8'),
    ...portableCommandContracts.map(({ file }) =>
      readFile(path.join(tauriRoot, 'permissions/autogenerated', file), 'utf8')),
  ]);
  portableCommandContracts.forEach(({ command, permission }, index) => {
    if (!new RegExp(`"${command}"`).test(buildScriptSource)) {
      fail(`Tauri build manifest must register ${command}`);
    }
    if (!new RegExp(`native_files::${command}\\b`).test(libSource)) {
      fail(`Tauri invoke handler must register ${command}`);
    }
    const permissionSource = portablePermissionSources[index];
    if (
      !permissionSource?.includes(`identifier = "${permission}"`)
      || !permissionSource.includes(`commands.allow = ["${command}"]`)
    ) {
      fail(`Generated permission must allow only ${command}`);
    }
  });
