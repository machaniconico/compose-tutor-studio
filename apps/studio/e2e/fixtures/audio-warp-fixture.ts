...
/** Canonical local-only mono fixture with two separated, stable notes. */
export function createAudioWarpPitchFixture(): Buffer {
  const firstFrames = Math.round(SAMPLE_RATE * 2);
  const silenceFrames = Math.round(SAMPLE_RATE * 0.08);
  const secondFrames = Math.round(SAMPLE_RATE * 2);
  ...
  for (let frame = 0; frame < frames; frame += 1) {
    const frequency = frame < firstFrames
      ? 440
      : frame >= firstFrames + silenceFrames
        ? 523.251
        : 0;
    const sample = frequency === 0
      ? 0
