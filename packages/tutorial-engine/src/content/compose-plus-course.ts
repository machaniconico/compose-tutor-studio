...
//   - ローカル作曲コーチ
//
// すべてのステップは既存の StepGoal 種別（event / project predicate / exercise）
// だけで達成判定でき、tutorial-engine の型・エンジンには手を入れていない。
...
export const COMPOSE_PLUS_LESSON_3: TutorialLesson = {
  ...
  steps: [
    ...
    },
    {
      ...
      title: 'スイングをかけて再生しよう',
      instruction:
        'ドラムの「スイング」を少し上げてから再生し、ノリの変化を聴いてみましょう。',
      explanation:
        'スイングは、裏拍（偶数の16分）の発音を少し後ろにずらすことで“ハネた”ノリを作ります。0のときは真っ直ぐ（マシン的）、上げるほどジャズやヒップホップのような揺れが出ます。かけすぎると重くなるので、少しずつが基本です。ヒューマナイズ（強さの揺らぎ）も足すと、より生っぽくなります。',
      goal: { kind: 'event', eventType: 'transport.played', count: 1 },
      hints: [
        'ドラムパネルの「スイング」スライダーを少し右へ動かしましょう。',
        '同じパターンでもスイングの有無で印象が大きく変わります。聴き比べてみましょう。',
      ],
    },
    {
...
export const COMPOSE_PLUS_LESSON_4: TutorialLesson = {
  ...
  steps: [
    ...
    },
    {
      ...
      title: 'エフェクトを足して再生しよう',
      instruction:
        'どれかのトラックに Reverb（響き）か Delay（やまびこ）を足して、再生して確認しましょう。',
      explanation:
        'Reverb は空間の“響き”を足して音を遠く・広く感じさせます。Delay は音を繰り返す“やまびこ”で、メロディに広がりを出せます。Filter は高音・低音を削って音の“明るさ”を調整します。かけすぎると濁るので、うっすらから始めるのがコツです。',
      goal: { kind: 'event', eventType: 'transport.played', count: 1 },
      hints: [
        'ミキサーのトラックでエフェクトを追加し、Reverb か Delay を選びましょう。',
        'wet（かかり具合）を少しだけ上げて、原音とのバランスを聴きましょう。',
      ],
    },
    {
